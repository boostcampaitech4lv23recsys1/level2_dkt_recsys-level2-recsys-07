...
class Saint(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.dec_comb_proj = nn.Linear((self.hidden_dim//3)*6, self.hidden_dim)
    ...
    def forward(self, input):
        test, question, tag, assessmentItemID_mean_answerCode, solving_time, _, mask, interaction = input
        ...
        solving_time = solving_time.unsqueeze(2).to('cuda')
        linear_cont = nn.Linear(solving_time.shape[-1], embed_test.shape[-1]).to('cuda') #딴놈 마지막사이즈 맞춰줘야함
        linear_solving_time = linear_cont(solving_time).to('cuda')
